...
// Image imports
import ImagemodalOne from "@/assets/Modules/Imagemodalseven.png";
    ...
    <div className="fixed inset-0 z-[200] flex md:items-baseline lg:items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm">
      ...
      >
        ...
        <div className="w-full flex justify-between items-center">
          <h1 className="font-semibold text-gray-700">Tilbake til Modul 07</h1>
          <button
            onClick={onClose}
...
export default function CarSceneMerged() {
  ...
  const { data, refetch } = useGetModulesByIdQuery("693670abf4d0d2d1e21e1d6d");
  ...
  const startLearningHandler = async () => {
    try {
      const introBody = { moduleNumber: 7, checkpoint: "intro_page", childProfileId: childId };
      await updateCheckPoints({ updatesBody: introBody }).unwrap();

      const learningBody = { moduleNumber: 7, checkpoint: "learning_content", childProfileId: childId };
      const response = await updateCheckPoints({ updatesBody: learningBody }).unwrap();

      if (response?.code === 200) {
         router.push(`/LearningSeven?childId=${childId}`);
      }
    } catch {
...
// import NRunModules07 from "@/assets/NModules/NRunModules07.png";
// import Imagemodalseven from "@/assets/Modules/Imagemodalseven.png";
// import tirProint from "@/assets/Modules/tirProint.png";
// import Group from "@/assets/Modules/Group.png";
// import Image from "next/image";
...
//                     `Modul 7: Selvstendig problemløsning`}
//                 </h2>
//               </div>
//               <div className="bg-white mt-2 p-2  rounded-md">
...
//                       `Ha verktøy klare for uventede situasjoner`}
//                   </h1>
//                 </div>
...
//                       <div
//                         key={objective.order || index}
//                         className="flex items-center space-x-2 py-1 rounded-xl w-fit"
//                       >
//                         <Image className="w-6 h-6" src={Group} alt="icon" />
