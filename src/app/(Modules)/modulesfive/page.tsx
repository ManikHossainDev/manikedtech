...
// Image imports
import ImagemodalOne from "@/assets/Modules/Imagemodalfive.png";
    ...
    <div className="fixed inset-0 z-[200] flex md:items-baseline lg:items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm">
      ...
      >
        ...
        <div className="w-full flex justify-between items-center">
          <h1 className="font-semibold text-gray-700">Tilbake til Modul 05</h1>
          <button
            onClick={onClose}
...
export default function CarSceneMerged() {
  ...
  const { data, refetch } = useGetModulesByIdQuery("69366d40f4d0d2d1e21e1d61");
  ...
  const startLearningHandler = async () => {
    try {
      const introBody = { moduleNumber: 5, checkpoint: "intro_page", childProfileId: childId };
      await updateCheckPoints({ updatesBody: introBody }).unwrap();

      const learningBody = { moduleNumber: 5, checkpoint: "learning_content", childProfileId: childId };
      const response = await updateCheckPoints({ updatesBody: learningBody }).unwrap();

      if (response?.code === 200) {
         router.push(`/Learningfive?childId=${childId}`);
      }
    } catch {
...
// import NRunModules05 from "@/assets/NModules/NRunModules05.png";
// import Imagemodalfive from "@/assets/Modules/Imagemodalfive.png";
// import tirProint from "@/assets/Modules/tirProint.png";
// import Group from "@/assets/Modules/Group.png";
// import Image from "next/image";
...
//   return (
//     <div
//       className="relative h-screen bg-contain bg-no-repeat bg-bottom xl:bg-cover xl:bg-bottom"
//       style={{
...
//         >
//           <div className="bg-[#E58A11]  px-1 md:py-2 rotate-3 xl:pt-[5px] xl:px-[12px] xxl:px-[16px] border border-white rounded-md relative h-full flex flex-col items-center justify-between">
//             {/* Title Section */}
...
//                       <button className="flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-semibold py-3 px-2 md:px-8 rounded-lg transition-colors duration-200 shadow-md">
//                         <FaStar className="text-xs md:text-xl" />
