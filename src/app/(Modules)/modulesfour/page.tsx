...
// Image imports
import ImagemodalOne from "@/assets/Modules/imagemodalfour.png";
    ...
    <div className="fixed inset-0 z-[200] flex md:items-baseline lg:items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm">
      ...
      >
        ...
        <div className="w-full flex justify-between items-center">
          <h1 className="font-semibold text-gray-700">Tilbake til Modul 04</h1>
          <button
            onClick={onClose}
...
export default function CarSceneMerged() {
  ...
  const { data, refetch } = useGetModulesByIdQuery("69366b5cf4d0d2d1e21e1d5b");
  ...
  const startLearningHandler = async () => {
    try {
      const introBody = { moduleNumber: 4, checkpoint: "intro_page", childProfileId: childId };
      await updateCheckPoints({ updatesBody: introBody }).unwrap();

      const learningBody = { moduleNumber: 4, checkpoint: "learning_content", childProfileId: childId };
      const response = await updateCheckPoints({ updatesBody: learningBody }).unwrap();

      if (response?.code === 200) {
         router.push(`/Learningfour?childId=${childId}`);
      }
    } catch {
...
// import NRunModules04 from "@/assets/NModules/NRunModules04.png";
// import imagemodalfour from "@/assets/Modules/imagemodalfour.png";
// import tirProint from "@/assets/Modules/tirProint.png";
// import Group from "@/assets/Modules/Group.png";
// import Image from "next/image";
...
//         >
//           <div className="bg-[#E58A11]  px-1 md:py-2 rotate-3 xl:pt-[5px] xl:px-[13px] xxl:px-[15px] border border-white rounded-md relative h-full flex flex-col items-center justify-between">
//             {/* Title Section */}
...
//                 Digitalt
//               </div>
//               <div className="md:text-[8px] xl:text-[12px] tracking-tight leading-tight font-extrabold">
//                 Dømmekraft
//               </div>
...
//                 />
//                 <h2 className=" text-base md:text-xl font-bold text-[#E58A11]">
//                   {selectedModule?.title || `Modul 4: Digital dømmekraft`}
//                 </h2>
//               </div>
//               <div className="bg-white mt-2 p-2  rounded-md">
...
//                       `Skille fakta fra fiksjon og reklame på nettet.`}
//                   </h1>
//                 </div>
...
//                         (obj: LearningObjective, index: number) => (
//                           <div
//                             key={index}
