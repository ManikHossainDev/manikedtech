...
// Image imports
import imagemodaltow from "@/assets/Modules/Imagemodalfive.png";
    ...
    <div className="fixed inset-0 z-[200] flex md:items-baseline lg:items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm">
      ...
      >
        ...
        <div className="w-full flex justify-between items-center">
          <h1 className="font-semibold text-gray-700">Tilbake til Modul 02</h1>
          <button
            onClick={onClose}
        ...
        <div className="mt-2">
          ...
          <div className="bg-white flex items-center space-x-2 w-fit py-3 px-2 rounded-br-[38px] rounded-tl-[38px]">
            <Image width={40} height={40} className="w-10 h-10" src={imagemodaltow} alt="imag" />
            <h2 className="text-base md:text-xl font-bold text-[#E58A11]">
              {selectedModule?.title || "Social Boundaries"}
            </h2>
          </div>
...
export default function CarSceneMerged() {
  ...
  const { data, refetch } = useGetModulesByIdQuery("695e2fe8eaf052fd70aa958a");
  console.log(data)
  ...
  const startLearningHandler = async () => {
    try {
      const introBody = { moduleNumber: 2, checkpoint: "intro_page", childProfileId: childId };
      await updateCheckPoints({ updatesBody: introBody }).unwrap();

      const learningBody = { moduleNumber: 2, checkpoint: "learning_content", childProfileId: childId };
      const response = await updateCheckPoints({ updatesBody: learningBody }).unwrap();

      if (response?.code === 200) {
        router.push(`/Learningtow?childId=${childId}`);
      }
    } catch {
  ...
  return (
    <div className={`w-full h-screen overflow-hidden bg-gradient-to-b ${bgClasses} relative font-sans transition-colors duration-1000`}>
      ...
      <div className="absolute top-6 left-4 z-50">
        <Button className="bg-[#FF9E1C] text-white font-semibold py-4">
          <Link href="/AddChildren" className="flex items-center text-xs md:text-md">
      ...
      <AnimatePresence>
        ...
        {carPhase === "stopped" && (
          ...
          >
            <button
              onClick={handleReset}
              className={`pointer-events-auto flex items-center gap-2 px-3 md:px-6 py-2 md:py-3 font-bold text-lg rounded-full shadow-lg transition-transform hover:scale-105 active:scale-95 cursor-pointer border ${isDay ? "bg-white hover:bg-gray-100 text-gray-800 border-gray-200" : "bg-white/10 hover:bg-white/20 backdrop-blur-md text-white border-white/20"}`}
            >
              <RotateCcw className="w-3 h-3 md:w-7 md:h-7 lg:w-8 lg:h-8 fill-current" />
...
// import NRunModules02 from "@/assets/NModules/NRunModules02.png";
...
// import { useGetModulesByIdQuery } from "@/redux/features/modules/modulesGet";
// import { LearningObjective } from "@/types/moduleTwo.types";
// import { useRouter, useSearchParams } from "next/navigation";
// import { useUpdateCheckPointsMutation } from "@/redux/features/modules/GetProgressOverview";
// import { Button } from "antd";
// import Link from "next/link";
...
//   useEffect(() => {
//     if(data?.data) dispatch(setSelectedModule(data.data)); // Set the data.data object into selectedModule
//   }, [data, dispatch]);
...
//         <div
//           className="absolute top-[15px] right-[100px] sm:top-[40px] sm:right-[140px] md:top-[40px] md:right-[265px] lg:top-[100px] lg:right-[360px]   xl:top-[260px] xl:right-[393px]  xxl:top-[295px] xxl:right-[593px] 3xl:top-[380px] 3xl:right-[780px] 4xl:top-[330px] 4xl:right-[875px] shadow-lg shadow-[#E58A11]/50 cursor-pointer"
//           onClick={handleDivClick}
//         >
//           <div className="bg-[#E58A11]  px-1 md:py-2 rotate-3 xl:pt-[5px] xl:px-[13px] xxl:px-[16px] border border-white rounded-md relative h-full flex flex-col items-center justify-between">
//             {/* Title Section */}
//             <div className="hidden md:block text-center text-white font-black w-full">
//               <div className="md:text-[10px] xl:text-[16px] 3xl:text-[20px] tracking-tight leading-tight font-extrabold">
//                 Sosial
//               </div>
//               <div className="md:text-[8px] 3xl:text-[18px] xl:text-[11px] tracking-tight leading-tight font-extrabold">
//                 Boundarie
//               </div>
...
//                 />
//                 <h2 className=" text-base md:text-xl font-bold text-[#E58A11]">
//                   {selectedModule?.title || "Tilbake til Modul 02"}
//                 </h2>
//               </div>
//               <div className="bg-white mt-2 p-2  rounded-md">
//                 <div className="flex items-center space-x-2 bg-[#FFDFD2] p-2 rounded-xl w-fit">
...
//                       `Hvem er en venn og hvem er en fremmed på nettet?.`}
//                   </h1>
//                 </div>
...
//                       (objective: LearningObjective, index: number) => (
//                         <div
//                           key={index}
//                           className="flex items-center space-x-2 py-1  rounded-xl w-fit"
//                         >
//                           <Image className="w-6 h-6 " src={Group} alt="imag" />
//                           <h1 className="mt-1">{objective.text}</h1>
//                         </div>
//                       )
//                     )}
//                   <div className="flex gap-2 md:gap-4 py-2 justify-center">
